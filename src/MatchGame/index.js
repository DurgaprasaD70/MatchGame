...
class MatchGame extends Component {
  state = {
    ...
    isGameInPregress: true,
    timerInseconds: 60,
    matchedImageObj: imagesList[0],
  }
  ...
  renderGameImages = () => {
    const {activeTabId, matchedImageObj} = this.state

    const showfinalImagesList = this.showImagesList()
    const matchedImage = matchedImageObj.imageUrl
        ...
        <ul className="tab-list-container">
          {tabsList.map(eachTab => (
            <MatchGameTabItem
              ...
              showImagesRelatedToTabId={this.showImagesRelatedToTabId}
            />
          ))}
        </ul>
        <ul className="thumbnail-images-container">
          {showfinalImagesList.map(eachImage => (
            <ThumbnailItem
              eachImage={eachImage}
              key={eachImage.id}
              addedToClickedList={this.addedToClickedList}
            />
          ))}
        </ul>
      </>
  ...
  renderScoreCard = () => {
    ...
    const score = clickedImagesList.length
    ...
    return (
      <div className="scorecard-bg-container">
        <img
          ...
          className="trophy"
        />
        <p className="your-scorecarc-text">YOUR SCORE</p>
  ...
  getShuffeledImagesList = () => {
    const result = imagesList[Math.floor(Math.random() * 30)]
    return result
  }

  addedToClickedList = id => {
    let shuffeledList = this.getShuffeledImagesList()
    const {clickedImagesList, matchedImageObj} = this.state
    const score = clickedImagesList.length
    const isPresent = clickedImagesList.includes(id)

    if (isPresent) {
      this.clearTimeInterval()
      this.setState({isGameInPregress: false})
    } else {
      if (clickedImagesList.includes(shuffeledList.id)) {
        shuffeledList = this.getShuffeledImagesList()
      } else {
        if (id === matchedImageObj.id) {
          this.setState(prevState => ({
            clickedImagesList: [...prevState.clickedImagesList, id],
            matchedImageObj: shuffeledList,
          }))
        } else {
          this.clearTimeInterval()
          this.setState({isGameInPregress: false})
  ...
  render() {
    const {isGameInPregress, timerInseconds, clickedImagesList} = this.state
    ...
    return (
      <>
        <div className="navbar-container">
          <Navbar
            isGameInPregress={isGameInPregress}
